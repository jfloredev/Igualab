// Vista Administrador: historial y descarga de reportes de prospección (RF-044 / RF-045).
Views.descargas = {
  html() {
    const rows = App.state.reports.map((r) => `
      <tr class="border-b border-surface-variant hover:bg-surface/50 transition-colors">
        <td class="py-md px-md"><div class="flex items-center gap-sm"><span class="material-symbols-outlined text-error">picture_as_pdf</span><span class="font-medium">Prospección · ${Helpers.esc(r.empresa)}</span></div></td>
        <td class="py-md px-md">${Helpers.esc(r.sector || "")}</td>
    ...
    return `
      ${Cards.sectionHeader("Descargar reportes", "Reportes de prospección generados, con la empresa, el año y la fecha de generación (RF-044). Disponibles aun si el asistente de IA no está operativo (RN-030).")}
      <div class="bg-surface-container-lowest rounded-xl border border-surface-variant shadow-sm overflow-hidden">
        <div class="overflow-x-auto">
          <table class="w-full text-left border-collapse">
            <thead><tr class="bg-surface-container-low border-b border-outline-variant">${["Reporte", "Sector", "Año", "Fecha", ""].map((h) => `<th class="py-sm px-md font-label-sm text-label-sm text-on-surface-variant uppercase ${h === "" ? "text-right" : ""}">${h}</th>`).join("")}</tr></thead>
